...
//		);
		...
		var inf={"doString":"Buy","userName":userName,"trainName":trainName,"startstation":startstation,"endstation":endstation,"tourist":tourist,"IDCard":IDCard,
				"phone":phone,"start_time":storage.start_time,"arrive_time":storage.arrive_time,"station_no":storage.station_no,"SeatName":SeatName
		};
		$.ajax({
			type:'post',
			url:'../BuyServlet',
			...
			success:function(data){
				...
				if(data){
					if(confirm("购票成功,单击确定跳转至个人界面查看购票信息")){
					...
					$("#dataTable").DataTable({
						...
						columns:[
							...
							columnDefs:[{
								...
								targets:8,
								render:function(data,type,full,meta){
									return '<button class="btn btn-primary" onclick="gm(this)">购买</button>';
								}
							},
							{
								targets:6,
